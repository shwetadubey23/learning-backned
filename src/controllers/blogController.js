    ...
    try {
        ...
        if (!title || !body || !authorId || !category)
            return res.status(400).send({
                status: false,
                msg: `Please fill the all required field(title, body, authorId, category`
            });

        const validateAuthorId = await AuthorModel.findById(authorId);
        if (!validateAuthorId)
            return res.status(404).send({ status: false, msg: "author deoesn't exist" });
    ...
    try {
        ...
        const updatedBlog = await BlogModel.findOneAndUpdate(
            { _id: blogId },
            {
                ...
                $set: {
                    title: details.title, body: details.body,
                    category: details.category, isPublished: true, publishedAt: Date.now()
                },
            }, { new: true }
        );
...
const deleteBlogByQueryParam = async (req, res) => {
    try {
        ...
        if (filterByQuery.isDeleted == true)
            return res.status(404).send({ status: false, msg: "blog already deleted" });

        let userId = req.validateToken.userId
        // console.log(userId)

        if (userId !== filterByQuery.authorId.toString())
